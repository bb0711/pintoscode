...
process_execute (const char *file_name)
{
  ...
  //printf("in process_execute: file_name = %s\n", file_name);
 ...
 //char * copy_file_name =(char *)malloc(len);
 strlcpy(copy_file_name, file_name, strlen(file_name) + 1);
 copy_file_name = strtok_r (copy_file_name, " ", &save_ptr);

 //printf("process_execute filename: %s, copy_filename: %s, tid: \n",file_name, copy_file_name);

 if(filesys_open(copy_file_name) == NULL)
    return -1;

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (copy_file_name, PRI_DEFAULT, start_process, fn_copy);
  pid = tid;
...
process_wait (tid_t child_tid UNUSED) 
{
   ...
   for (e = list_begin (&cur->child_list); e != list_end (&cur->child_list); e = list_next (e)){
        ...
        if (child_process->tid == child_tid){
           // check if you wait more than one. if you already waited on this pid => return -1 !!
           if( list_size(&(&child_process->sema_child)->waiters) >=1 )
                return -2;

           goto done;
        }
   }
   //if there are no such child in the thread's child_list
   //printf("no such child: %d\n",child_tid);
   return -1;

   done:
        sema_down(&(child_process->sema_child));
        int exitnum = child_process->exit_num;

        list_remove(&(child_process->child_elem));
        sema_up(&child_process->exit_lock);
        return exitnum;
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* this is for parsing */
   ...
   char *copy_file_name =(char *)malloc(len);
   strlcpy(copy_file_name, file_name, len);
   copy_file_name = strtok_r (copy_file_name, " ", &save_ptr);
...
validate_segment (const struct Elf32_Phdr *phdr, struct file *file) 
{
  ...
  if (phdr->p_vaddr < PGSIZE)//if (phdr->p_offset < PGSIZE)
    return false;
...
setup_stack (void **esp, const char *file_name)
{
  ...
  if (fte != NULL)
    ...
    char * copy_file_name =(char *)malloc(len);
    char ** token_arr;
    int *addr_arr[MAX_ARGC];
    int argc=0;

    token_arr = (char **)malloc( MAX_ARGC * sizeof(char *));

    strlcpy(copy_file_name, file_name, len);

    for (token = strtok_r (copy_file_name, " ", &save_ptr); token != NULL ; token = strtok_r (NULL, " ", &save_ptr)){
        //strlcpy(&token_arr[argc], token, strlen(token)+1);
        token_arr[argc] = token;
        //printf("parsing: token_arr[%d]: %s, %d\n", argc, token, strlen(token)+1);
        argc ++;
    }
    ...
    if( argc > 0){
        for(i = 1; i<=argc; i++){
            *esp -= strlen(token_arr[argc-i]) +1;
            strlcpy(*esp, token_arr[argc-i], strlen(token_arr[argc-i])+1);
            addr_arr[argc-i]= *esp;
        }
    }
    uint8_t temp_0 =0;
    while( (int)*esp % 4 !=0){
        *esp -= 1;
        memcpy(*esp, &temp_0, sizeof(uint8_t));
    }
    int temp = 0;
    *esp-=4;
    ...
    for(i=1; i<=argc; i++){
        *esp-=4;
        memcpy(*esp, &addr_arr[ argc - i ], sizeof(char *));
    }
    int temp_addr= *esp;
    *esp-=4;
    memcpy(*esp, &temp_addr, sizeof(char **));
